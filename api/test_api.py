...
@app.delete("/job-requirements/{filename}")
async def delete_job_requirements(filename: str, background_tasks: BackgroundTasks):
    """Delete a job requirements PDF file"""
    global job_desc_path, ranked_cvs, faiss_index, metadata
    ...
    try:
        ...
        file_path = Path("jobs") / filename
        
        # Check if file exists
        if not file_path.exists():
            raise HTTPException(status_code=404, detail=f"Job file {filename} not found")
            
        # Check if this is the current job description
        is_current = str(file_path) == job_desc_path
        
        # Delete the file
        file_path.unlink()
        
        # If this was the current job, update the system
        if is_current:
            # Reset the current job path
            job_desc_path = None
            
            # Reinitialize the system if we have CVs
            if faiss_index is not None and metadata is not None:
                try:
                    ranked_cvs = rank_cvs(None, faiss_index, metadata)
                except Exception as e:
                    print(f"Error updating rankings after job deletion: {str(e)}")
        
        return {
            "status": "success",
            "message": f"Job file {filename} deleted successfully",
        ...
        raise HTTPException(status_code=500, detail=f"Error deleting job file: {str(e)}")
